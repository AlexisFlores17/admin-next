...
const getProductInCar = (cart: { [id: string]: number }) => {
...
export default function CartPage() {
  ...
  const productInCart = getProductInCar(cart);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full">
        ...
        <div className="flex flex-col w-full sm:w-4/12"></div>
        <WidgetItem title="Total a pagar">
          <div className="mt-2 flex justify-center gap-4">
            <h3 className="text-3xl flex justify-center gap-4">${(totalToPay * 1.15).toFixed(2)}</h3>
          </div>
